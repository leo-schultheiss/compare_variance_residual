from __future__ import division  # Functions to estimate cost for each lambda, by voxel
...
def residuals_textual(source_train, source_val, target_data, eachfeature):
    ...
    for eachlayer in np.arange(12):
        split_num = 0
        # for BERT
        y_train = np.vstack([zscore(target_data.item()[story][eachlayer][5 + trim:-trim]) for story in
                             list(target_data.item().keys())[:-1]])
        y_test = np.vstack([zscore(target_data.item()[story][eachlayer][5 + trim:-trim]) for story in
                            list(target_data.item().keys())[-1:]])
...
def residuals_visual(source_train, source_val, target_data, eachfeature):
    ...
    for eachlayer in np.arange(12):
        ...
        # weights, lbda = cross_val_ridge(np.nan_to_num(sourcedata_train),np.nan_to_num(y_train))
        y_pred1, y_pred2 = kernel_ridge(np.nan_to_num(sourcedata_train), np.nan_to_num(sourcedata_test), y_train,
                                        y_test)
