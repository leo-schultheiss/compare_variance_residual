...
def predict_joint_model(data_dir, context_representations, subject_num, modality, num_layers, low_level_feature,
                        output_dir):
    ...
    z_base_feature_train = np.vstack(
        [zscore(base_features_train[story][low_level_feature][5 + trim:-trim]) for story in
         base_features_train.keys()])
    z_base_feature_val = np.vstack(
        [zscore(base_features_val[story][low_level_feature][5 + trim:-trim]) for story in
    ...
    # join input features (context representations and low-level textual features)
    Rstim = [np.hstack((delayed_Rstim[layer_number], z_base_feature_train)) for layer_number in np.arange(num_layers)]
    Pstim = [np.hstack([delayed_Pstim[layer_number], z_base_feature_val]) for layer_number in np.arange(num_layers)]
...
if __name__ == '__main__':
    ...
    predict_joint_model(args.data_dir, args.context_representations, args.subjectNum, args.modality, args.layers,
