...
def prediction_joint_model(Rstim_1, Rstim_2, Pstim_1, Pstim_2, data_dir, subject, modality, num_layers=12):
    """
    Train a joint model for two feature spaces
    :param Rstim_1 Training stimuli with TR time points and N features. Each feature should be Z-scored across time
    :param Rstim_1 Training stimuli with TR time points and M features. Each feature should be Z-scored across time
    :param Pstim_1 Test stimuli with TP time points and O features. Each feature should be Z-scored across time
    :param Pstim_1 Test stimuli with TP time points and P features. Each feature should be Z-scored across time
    ...
    for layer_number in np.arange(num_layers):
        ...
        Rstim = np.hstack((Rstim_1[layer_number], Rstim_2))
        Pstim = np.hstack([Pstim_1[layer_number], Pstim_2])
        ...
        pred = np.dot(np.nan_to_num(Pstim), wt)
        print(pred.shape)
        print(zPresp.shape)
        ...
        for voxel_index in range(zPresp.shape[1]):
            voxelwise_correlations[voxel_index] = np.corrcoef(zPresp[:, voxel_index], pred[:, voxel_index])[0, 1]
        print(voxelwise_correlations)
        correlations_per_layer.append(voxelwise_correlations)
    return correlations_per_layer, pred
...
if __name__ == '__main__':
    ...
    voxelxise_correlations, predictions = prediction_joint_model(delayed_Rstim, z_base_feature_train,
                                                                 delayed_Pstim, z_base_feature_val,
                                                                 args.data_dir,
                                                                 subject,
                                                                 args.modality, args.layers)
    ...
    for layer in range(len(voxelxise_correlations)):
        np.save(
            os.path.join(str(main_dir), args.modality, f"joint_model_prediction_voxelwise_correlation_layer{layer}"),
