...
def run_ridge_pipeline(X, Y, n_samples_train, run_onsets, alphas, number_of_delays, n_targets_batch, n_alphas_batch, n_targets_batch_refit, score_func=himalaya.scoring.r2_score):
    """
    Run ridge pipeline using RidgeCV
    """
    delayer = Delayer(delays=range(1, number_of_delays + 1))
    solver_params = dict(n_targets_batch=n_targets_batch, n_alphas_batch=n_alphas_batch, n_targets_batch_refit=n_targets_batch_refit, score_func=score_func)
...
def run_banded_pipeline(Xs, n_features_list, Y, n_samples_train, run_onsets, alphas, n_iter, number_of_delays, n_targets_batch, n_alphas_batch, n_targets_batch_refit, score_func=himalaya.scoring.r2_score):
    """
    ...
    solver_params = dict(alphas=alphas, n_iter=n_iter, n_targets_batch=n_targets_batch, n_alphas_batch=n_alphas_batch, n_targets_batch_refit=n_targets_batch_refit, score_func=score_func)
    cv = generate_leave_one_run_out(n_samples_train, run_onsets)
